...
3. Press **读取账号池** to load invalid CPA accounts and the Hotmail account pool from the Web app.
...
The CPA Codex Web page must answer `window.postMessage` requests from `content-cpa.js`.
...
- Actions: `GET_CAPABILITIES`, `GET_ACCOUNT_POOLS`, `BUILD_QUEUE`, `GET_QUEUE`, `CLAIM_JOB`, `UPDATE_JOB`, `START_JOB_OAUTH`, `FETCH_CODE`, `SUBMIT_CALLBACK`, `CHECK_OAUTH_STATUS`, `RELEASE_JOB`

## Safety Notes

- Default host permissions are limited to local Web URLs and OpenAI auth hosts. Remote CPA Web hosts are requested as optional host permissions only after saving a platform binding URL.
