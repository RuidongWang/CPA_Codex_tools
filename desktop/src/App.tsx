...
function resolveErrorMessage(error: unknown, fallback: string): string {
  // 桌面端统一把异常压成可读文案，避免界面上出现空消息或生硬对象字符串。
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      try {
        ...
        setErrorMessage(resolveErrorMessage(error, "初始化桌面端失败"));
        setBusyMode("idle");
      }
    }
  ...
  // 只有“全量备份时间晚于最后一次草稿变更”才算新备份，局部备份不能替代同步前兜底。
  ...
  const backupButtonLabel = selectedCount > 0
    ? `${desktopRuntime ? "备份" : "下载"}选中 (${selectedCount})`
    : `${desktopRuntime ? "备份" : "下载"}所有账号`;
  ...
  function flushQueuedProgress() {
    // 备份和同步的真实事件有时会在结束前密集涌入，这里改成逐步追帧，避免 0 直接跳到完成。
  ...
  async function handleSaveSettings(settings: { backupPath: string; queryConcurrency: number }) {
    const nextConfig = {
      ...config,
      ...settings,
    };
  ...
  async function runBackupAccounts(
    ...
    // 手动备份和“同步前先备份”共用同一条链路，避免两套状态机越改越散。
    if (!targetItems.length) {
      return false;
    }
    if (desktopRuntime && !config.backupPath.trim()) {
      setErrorMessage("请先在设置里配置账号备份路径");
      setLoadingLabel("缺少账号备份路径");
      return false;
    }
    ...
    try {
      ...
      const downloaded = await downloadSelectedAccounts(config, targetItems, (event) => {
        showProgress({
          ...
          completed: event.completed,
          total: event.total,
          currentLabel: event.currentLabel || `已备份 ${event.completed} / ${event.total}`,
          elapsedLabel: "",
        });
      });
      ...
      showProgress({
        ...
        total: targetItems.length,
        currentLabel: "本轮备份已结束",
        elapsedLabel: formatElapsedLabel(performance.now() - startedAt),
      });
      setLoadingLabel(options.successLabel(downloaded.length));
      return true;
    } catch (error) {
      setErrorMessage(resolveErrorMessage(error, "备份账号配置失败"));
      setLoadingLabel("备份账号配置失败");
      patchVisibleProgress((current) =>
        current
          ? {
              ...current,
  ...
  async function handleBackupAccounts() {
    ...
    await runBackupAccounts(selectedBackupItems, {
      ...
      successLabel: (count) => (selectedCount > 0 ? `已备份 ${count} 个选中账号配置` : `已备份 ${count} 个账号配置`),
    });
  }
  ...
  async function handleBackupThenSync() {
    ...
    const backedUp = await runBackupAccounts(allItems, {
      ...
      successLabel: (count) => `已备份 ${count} 个账号配置，继续同步优先级`,
    });
    if (!backedUp) {
      return;
    }
  ...
  return (
    <div className="stitch-shell">
      ...
      <SettingsPanel
        ...
        busy={isBusy}
        usesBrowserDownload={!desktopRuntime}
