...
describe("SettingsPanel", () => {
  it("只保留备份路径和并发数两个本地设置项", () => {
    ...
    expect(within(dialog).getByLabelText("账号备份路径")).toBeInTheDocument();
  ...
  it("保存时会沿用现有优先级顺序，只回传本地设置字段的新值", async () => {
    ...
    await user.clear(screen.getByLabelText("账号备份路径"));
    await user.type(screen.getByLabelText("账号备份路径"), "E:\\archive\\codex");
    await user.clear(screen.getByLabelText("并发数"));
    await user.type(screen.getByLabelText("并发数"), "4");
    await user.click(screen.getByRole("button", { name: "保存设置" }));

    expect(onSave).toHaveBeenCalledWith({
      backupPath: "E:\\archive\\codex",
      queryConcurrency: 4,
    });
  });
