...
vi.mock("./lib/api", () => ({
  ...
  // App 单测默认走桌面模式，避免浏览器下载分支影响既有桌面交互断言。
  isTauriRuntime: mockApi.isTauriRuntime,
}));
...
beforeEach(() => {
  ...
  // 既有 App 用例锁住桌面端行为，浏览器模式单独放在 api-web.test.ts 覆盖。
...
describe("App", () => {
  ...
  it("标题栏只保留主标题，不再显示副标题", async () => {
    ...
    expect(screen.queryByText("生产巡检桌面台")).not.toBeInTheDocument();
  });
  ...
  it("设置面板可以保存备份路径和并发数", async () => {
    ...
    const backupInput = screen.getByLabelText("账号备份路径");
    ...
    await user.type(backupInput, "D:\\backup\\codex");
  ...
  it("同步到远端在缺少新备份时会弹出确认对话框", async () => {
    ...
    expect(screen.getByRole("button", { name: "先备份全部再同步" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "我已备份，直接同步" })).toBeInTheDocument();
  });
  ...
  it("同步前确认允许跳过备份直接同步", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: "我已备份，直接同步" }));
  ...
  it("同步前确认可以先执行全量备份再继续同步", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "生成本地草稿" }));
    await user.click(screen.getByRole("button", { name: "同步到远端" }));
    await user.click(await screen.findByRole("button", { name: "先备份全部再同步" }));
  ...
  it("备份到本地会按账号逐步更新进度条，并在完成前保持常驻", async () => {
    const user = userEvent.setup();
    ...
    render(<App />);
    await screen.findByText("free@example.com");
    await user.click(screen.getByRole("button", { name: "备份所有账号" }));

    expect(await screen.findByText("远端账号备份进度")).toBeInTheDocument();
  ...
  it("备份进度事件密集到达时也会显示中间步进", async () => {
    ...
    act(() => {
      screen.getByRole("button", { name: "备份所有账号" }).click();
    });
    expect(screen.getByText("远端账号备份进度")).toBeInTheDocument();
    ...
    expect(screen.getByText("本轮备份已结束")).toBeInTheDocument();
  });
  ...
  it("勾选账号后会改成备份选中，并只下载选中的远端账号", async () => {
    const user = userEvent.setup();
    mockApi.loadRuntimeConfig.mockResolvedValueOnce({
      cpaBaseUrl: "https://cpa.example/",
      managementKey: "demo-key",
    ...
    await user.click(screen.getByRole("checkbox", { name: "选择 free@example.com" }));
    await user.click(screen.getByRole("checkbox", { name: "选择 team-a@example.com" }));
    await user.click(screen.getByRole("button", { name: "备份选中 (2)" }));
  ...
  it("优先级按钮沿用主操作按钮配色", async () => {
    ...
    expect(screen.getByRole("button", { name: "备份所有账号" })).toHaveClass("command-button--primary");
